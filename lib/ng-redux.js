...
function ngReduxProvider() {
  ...
  this.connectToStore = function (store, middlewares) {
    invariant(isArray(storeEnhancers), 'The store parameter passed to connectToStore must be an Object. Instead received %s.', typeof store === 'undefined' ? 'undefined' : _typeof(store));

    invariant(!middlewares || isArray(storeEnhancers), 'The middlewares parameter passed to connectToStore must be an Array. Instead received %s.', typeof middlewares === 'undefined' ? 'undefined' : _typeof(middlewares));
